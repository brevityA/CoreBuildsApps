...
# The request screen builds its own prefill URLs on device, so the pieces it
# needs are generated into a resource rather than retyped into Kotlin. Same
# reasoning as the README block: a renamed form file or field id has to fail a
# gate here, not silently produce a link that 404s or fills nothing.
ANDROID_RES = ROOT / "app/src/main/res/values/issue_forms.xml"
# Which form answers which situation, matched on the file name so renumbering
# the forms does not silently swap them.
ANDROID_FORMS = {"missing": "new_icon_request", "mapping": "not_applying"}
# The two fields the app can always fill, because it is reading them off the
# device. Both forms must carry both, or the screen has nothing to say.
ANDROID_FIELDS = {"app": "app_name", "component": "component"}


def android_value(text: str) -> str:
    """Escape one string for an Android resource.

    Android trims leading and trailing whitespace from an unquoted value, and
    both issue titles end in a space — losing it yields `[Icon]Sparkle TV`.
    """
    escaped = (text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
               .replace("'", "\\'").replace("\"", "\\\""))
    return f'"{escaped}"' if text != text.strip() else escaped


def android_resource(templates: list[dict]) -> str:
    """The generated `values/issue_forms.xml`."""
    chosen: dict[str, dict] = {}
    for key, marker in ANDROID_FORMS.items():
        matches = [t for t in templates if marker in t["slug"]]
        if len(matches) != 1:
            fail(f"expected exactly one issue form matching {marker!r} for the "
                 f"request screen's {key} case, found {len(matches)}")
        chosen[key] = matches[0]
    for key, template in chosen.items():
        for field in ANDROID_FIELDS.values():
            if field not in template["prefillable"]:
                fail(f"{template['file']}: the request screen needs `{field}` to be a "
                     f"prefillable field (it has: {', '.join(template['prefillable'])})")

    lines = [
        '<?xml version="1.0" encoding="utf-8"?>',
        "<!-- Generated by tools/build_issue_prefills.py from"
        " .github/ISSUE_TEMPLATE/. Do not edit by hand. -->",
        "<resources>",
        f'    <string name="issue_endpoint" translatable="false">{ISSUES_NEW}</string>',
    ]
    for key, field in sorted(ANDROID_FIELDS.items()):
        lines.append(f'    <string name="issue_field_{key}" translatable="false">'
                     f"{android_value(field)}</string>")
    for key in ANDROID_FORMS:
        template = chosen[key]
        lines += [
            "",
            f'    <string name="issue_{key}_template" translatable="false">'
            f"{android_value(template['file'])}</string>",
            f'    <string name="issue_{key}_labels" translatable="false">'
            f"{android_value(','.join(template['labels']))}</string>",
            f'    <string name="issue_{key}_title" translatable="false">'
            f"{android_value(template['title'])}</string>",
        ]
    lines += ["</resources>", ""]
    return "\n".join(lines)


def check_android(templates: list[dict]) -> None:
    if not ANDROID_RES.exists():
        fail(f"missing {ANDROID_RES.relative_to(ROOT)}; run "
             "python tools/build_issue_prefills.py")
    if ANDROID_RES.read_text(encoding="utf-8") != android_resource(templates):
        fail(f"{ANDROID_RES.relative_to(ROOT)} is stale; run "
             "python tools/build_issue_prefills.py and commit it")
    print(f"request-screen resource matches {len(ANDROID_FORMS)} issue forms")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ANDROID_RES.write_text(android_resource(templates), encoding="utf-8")
    ...
    print(f"{ANDROID_RES.relative_to(ROOT)} written for the request screen")
    return 0

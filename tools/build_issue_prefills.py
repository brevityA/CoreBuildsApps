...
AUDIT_RES = (ROOT / "app" / "src" / "main" / "res" / "values" / "issue_prefill.xml")
# The in-app auditor's three dynamic values, in fmt-argument order.
AUDIT_ARGS = (("app_name", "%1$s"), ("component", "%2$s"), ("notes", "%3$s"))
# The same three values for a row whose package the pack already maps under a
# different activity: an icon exists and is not applying, which is the
# mapping form's job, and that form names the device field `device`.
AUDIT_MAPPING_ARGS = (("app_name", "%1$s"), ("component", "%2$s"),
                      ("device", "%3$s"))
...
def audit_resource(templates: list[dict],
    ...
    URL in strings.xml would rot in exactly the two ways this generator
    exists to prevent: the `template=` slug and the field ids. So the fmt
    string is generated from the form like every other link here — static
    parts (template, labels, title prefix) baked in, and three positional
    arguments the app URL-encodes and substitutes at scan time: %1$s the app
    ...
    """
    missing = _audit_fmt(
        next((t for t in templates if "new_icon_request" in t["slug"]),
             templates[0]),
        AUDIT_ARGS)
    mapping = _audit_fmt(resolve(templates, "icon_not_applying"),
                         AUDIT_MAPPING_ARGS)
    return (
        '<?xml version="1.0" encoding="utf-8"?>\n'
        "<!-- Generated by tools/build_issue_prefills.py. Do not edit by hand. -->\n"
        "<resources>\n"
        ...
        f'    <string name="audit_issue_url_fmt">{missing}</string>\n'
        "    <!-- The same link for an app the pack maps under another\n"
        "         activity: the icon exists and is not applying, so it opens\n"
        "         the not-applying form. Same three arguments, same escaping. -->\n"
        f'    <string name="audit_mapping_url_fmt">{mapping}</string>\n'
...
def _audit_fmt(form: dict, args: tuple[tuple[str, str], ...]) -> str:
    """One auditor deep link as an XML-escaped format string.

    `args` pairs each form field id with the positional placeholder the app
    fills; %1$s also completes the title. A field the form does not offer as
    input/textarea is left out, the same rule the README links live under.
    """
    params: list[tuple[str, str]] = [
        ("assignees", ""),
        ("labels", ",".join(form["labels"])),
        ("projects", ""),
        ("template", form["file"]),
        ("title", None),  # placeholder handled below: prefix + %1$s
    ]
    for field_id, ph in args:

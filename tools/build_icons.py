...
from glyphs import GLYPHS, render_svg  # noqa: E402
...
def main():
    ...
    for i in icons:
        ...
        for comp in i["components"]:
            lines.append(f'    <item component="ComponentInfo{{{esc(comp)}}}" '
                         f'drawable="{i["drawable"]}"/>')
            comp_count += 1
    lines.append('</resources>')
    write(XML_DIR / "appfilter.xml", "\n".join(lines) + "\n")
    print(f"\u2713 appfilter.xml written \u2014 {comp_count} components "
          f"\u2192 {len(icons)} drawables")

    # 4. drawable.xml — the in-app browser grid
    d = ['<?xml version="1.0" encoding="utf-8"?>',
         '<!-- Generated by tools/build_icons.py. Do not edit by hand. -->',
         '<resources>', '    <category title="Core Builds" />']
    for i in icons:

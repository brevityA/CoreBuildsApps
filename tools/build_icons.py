...
def main():
    ...
    # 4. drawable.xml — the in-app browser grid
    d = ['<?xml version="1.0" encoding="utf-8"?>',
         '<!-- Generated by tools/build_icons.py. Do not edit by hand. -->',
         '<resources>',
         '    <category title="Core Builds \u00b7 Banners (16:9)" />']
    # 16:9 banners are separate drawables. Listing them makes each one
    # selectable from the launcher's icon browser, so a user can pick the
    # banner treatment per app without changing the pack default.
    # Banners are the default, so they head the browser list; the square set
    # follows as the opt-in alternative.
    for i in icons:
        d.append(f'    <item drawable="{i["drawable"]}_banner" />')
    d.append('    <category title="Core Builds \u00b7 Square icons" />')
    for i in icons:

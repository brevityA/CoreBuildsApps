...
import {
  validate, issueBody, rateLimitKey, rateLimited, appJwt,
  handle, TITLE_PREFIX, ISSUE_LABEL,
} from "./worker.mjs";
...
test("issue body mirrors the issue form's text fields, in form order", () => {
  const yml = readFileSync(
    ...
    "utf8");
  // label: values of the form's input/textarea/dropdown rows, in file order.
...
test("rate limit trips after the hourly cap, missing KV fails open", async () => {
  const kv = mapKv();
  const env = { RATE_KV: kv, RATE_LIMIT_PER_HOUR: 3 };
  assert.equal(await rateLimited(env, "9.9.9.9"), false);
  assert.equal(await rateLimited(env, "9.9.9.9"), false);
  assert.equal(await rateLimited(env, "9.9.9.9"), false);
  assert.equal(await rateLimited(env, "9.9.9.9"), true);
  assert.equal(await rateLimited({}, "9.9.9.9"), false);
});
...
// A primed token cache so installationToken() never reaches for a JWT in
// request-flow tests: the access_tokens stub stays as a tripwire instead.
...
/** fetch stub that impersonates the GitHub API surface the worker touches. */
...
test("405 for non-POST, 400 for bad JSON and bad payloads", async () => {
...
test("files a new issue when no duplicate exists", async () => {
  ...
  globalThis.fetch = stub.f;
  try {
    const res = await handle(req(OK), ENV);
...
test("a duplicate press comments instead of opening a second issue", async () => {
  ...
  globalThis.fetch = stub.f;
  try {
    const res = await handle(req(OK), { ...ENV, RATE_KV: primed() });
...
test("rate-limited triple-press answers 429 without touching GitHub", async () => {
  ...
  await kv.put("gh:installation_token", "tok");
  const stub = githubStub();
  const realFetch = globalThis.fetch;
  globalThis.fetch = stub.f;
  try {
    assert.equal((await handle(req(OK), env)).status, 201);
    const second = await handle(req(OK), env);

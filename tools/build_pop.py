...
MIRROR_FILES = [
    ...
    "values/dimens.xml",
    # Generated by tools/build_issue_prefills.py from .github/ISSUE_TEMPLATE/.
    # The request screen is shared Kotlin, so Pop resolves the same names; the
    # byte comparison in tests/test_pop.py then keeps both packs pointing at
    # whatever the issue forms currently are.
    "values/issue_forms.xml",
]

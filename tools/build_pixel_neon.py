...
def palette_for(icon: dict) -> dict[str, str]:
    # No positional input anywhere below this line: a row's palette must be a
    # pure function of the row, or adding one catalog entry re-paints every
    # entry after it (#111).
    seed = f"{icon['drawable']}:{icon['name']}"
...
class SpritePainter:
    ...
    def signature(self, slot: int, attempt: int = 0):
        """A tiny deterministic sprite signature; also resolves hash collisions."""
        x = 2 + ((slot * 7 + attempt * 3) % 28)
        y = 2 + ((slot * 11 + attempt * 5) % 28)
        self.dot(x, y, self.secondary if (slot + attempt) % 2 else self.highlight)
        if (slot + attempt) % 3 == 0 and x < 30:
...
def choose_motif(icon: dict, rng: random.Random) -> str:
...
def brand_symbol(p: SpritePainter, icon: dict) -> None:
...
def render_banner(sprite, palette: dict[str, str], icon: dict):
    from PIL import Image, ImageDraw

    seed = hashlib.sha1(f"banner:{icon['drawable']}".encode()).digest()
    layout = seed[0] % 3
...
def main() -> int:
    ...
    for icon in icons:
        # Everything that shapes a sprite derives from the row's own content —
        # never from where the row sits in the catalog. Adding one icon must
        # not re-roll the art of every icon after it (#111). `slot` is the
        # stable per-row number the collision retry needs.
        slot = int.from_bytes(
            hashlib.sha256(f"slot:{icon['drawable']}".encode()).digest()[:4], "big") % 997
        seed_text = f"{icon['drawable']}:{icon['name']}:{icon.get('category', 'APP')}"
        seed = int.from_bytes(hashlib.sha256(seed_text.encode()).digest()[:8], "big")
        palette = palette_for(icon)
        painter = SpritePainter(palette, seed)
        motif_name = choose_motif(icon, painter.rng)
        if motif_name == "brand":
            brand_symbol(painter, icon)
        else:
        ...
        while pixel_hash in used_hashes:
            painter.signature(slot, attempt)
            final, small, mask = render_sprite(painter)
            pixel_hash = hashlib.sha256(final.tobytes()).hexdigest()
            attempt += 1
            if attempt > 8:
        ...
        write(PNG_DIR / f"{icon['drawable']}_banner.png",
              _png_bytes(render_banner(small, palette, icon)))
    ...
    receipt = {
        ...
        "wallpaperStyle": "original hybrid 8-bit cyber-noir environment scenes",

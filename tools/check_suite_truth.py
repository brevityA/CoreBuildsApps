...
SUITE_HUB_RES = ROOT / "app" / "src" / "main" / "res" / "values" / "suite_hub.xml"


def suite_hub_xml(suite: dict) -> str:
    """The companion-app table the in-app suite hub reads.

    suite.json is a build-time registry and is not bundled into the APK, so
    the hub gets its own generated resource instead: names, applicationIds
    (the `<queries>` targets and launch probes) and Downloader codes, in
    name order so the file is diff-stable. A code the registry still holds
    as `[USER TO SUPPLY]` becomes an empty item - a hub that printed the
    placeholder would be sending people to type nonsense into Downloader.
    """
    rows = sorted(
        (app.get("name", key), app.get("applicationId", ""),
         app.get("downloader", ""))
        for key, app in suite["apps"].items() if key != "iconpack"
    )
    ...
    def array(name: str, values: list) -> str:
        items = "".join(f"        <item>{escape(v)}</item>\n" for v in values)
        return f'    <string-array name="{name}">\n{items}    </string-array>\n'

    codes = ["" if code.startswith("[") else code for _, _, code in rows]
    return (
        '<?xml version="1.0" encoding="utf-8"?>\n'
        "<!-- Generated by tools/check_suite_truth.py in write mode. "
        "Do not edit by hand. -->\n"
        "<resources>\n"
        + array("suite_hub_names", [n for n, _, _ in rows])
        + array("suite_hub_pkgs", [p for _, p, _ in rows])
        + array("suite_hub_codes", codes)
        + "</resources>\n"
    )


def check_suite_hub(suite: dict) -> None:
    expected = suite_hub_xml(suite)
    write = "--write" in sys.argv
    if not SUITE_HUB_RES.is_file():
        if write:
            SUITE_HUB_RES.write_text(expected, encoding="utf-8")
            print(f"wrote {SUITE_HUB_RES.relative_to(ROOT)}")
            return
        fail("suite hub resource missing; run "
             "python tools/check_suite_truth.py --write")
    if SUITE_HUB_RES.read_text(encoding="utf-8") != expected:
        if write:
            SUITE_HUB_RES.write_text(expected, encoding="utf-8")
            print(f"refreshed {SUITE_HUB_RES.relative_to(ROOT)}")
            return
        fail("suite hub resource is stale against suite.json; run "
             "python tools/check_suite_truth.py --write")

...
object WallpaperDownloader {
    ...
    fun fetchUrl(
        ...
        require(cacheName.matches(Regex("[A-Za-z0-9._-]+"))) { "Invalid wallpaper cache name" }
    ...
    private fun download(
        ...
        try {
            ...
            if (total > MAX_BYTES) throw IllegalStateException("Wallpaper exceeds ${MAX_BYTES / 1024 / 1024}MB")
            conn.inputStream.use { input ->
                tmp.outputStream().use { out ->
                    ...
                    while (true) {
                        ...
                        if (n == -1) break
                        received += n
                        if (received > MAX_BYTES) throw IllegalStateException("Wallpaper exceeds ${MAX_BYTES / 1024 / 1024}MB")
                        out.write(buf, 0, n)
            ...
            val ratio = bounds.outWidth.toDouble() / bounds.outHeight.coerceAtLeast(1)
            if (bounds.outWidth < 1280 || bounds.outHeight < 720 || ratio !in 1.70..1.82) {
                tmp.delete()
                throw IllegalStateException("Downloaded file is not a valid 16:9 wallpaper")
            }

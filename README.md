...
> |---|---:|---|---|---|
> | **[Core Builds Icon Pack](#-icon-pack)** | `v1.9.1` | 943 transparent icons + 84 wallpapers for Projectivy Launcher | `5270601` | [`v*` / `iconpack`](../../releases) |
> | **[Core Builds Pixel Neon](#-pixel-neon-icon-pack)** | `v0.1.0` | 943 transparent 8-bit neon icons + 70 wallpapers for Projectivy Launcher | `[USER TO SUPPLY]` | [`pixel-neon-v*` / `pixel-neon`](../../releases) |
> | **[Core Builds Pop](#-core-builds-pop)** | `v1.0.0` | The same 943 icons, pop-art cartoon: 16 swatches, one container | `[USER TO SUPPLY]` | [`pop-v*` / `pop`](../../releases) |
...
`943 icons` · `84 wallpapers` · `v1.9.1`
...
943 icons across 21 categories — streaming, media centres, debrid services, players, launchers, tools, stores, live TV, music, sport, gaming, VPN, browsers, files, and more.
...
84 curated wallpapers in six active series — browse in the Wallpapers tab, preview full-screen, Set as device wallpaper or Save to `Pictures/CoreBuilds`. Multi-select export lets you bulk-save to a folder any launcher (or the system wallpaper picker) can rotate from. Thumbnails are bundled; the full images download on demand from GitHub, at the resolution each series was authored at (`1376×768` for series 6, 4K for series 1–3 and 7–8). Every active series now contains 12 wallpapers (or 24 for the original double set); series 8 adds twelve exact-black AMOLED designs; series 4 "Core Mark" was retired in v1.8.6 — see [`Wallpapers/README.md`](Wallpapers/README.md).
...
`943 icons` · `70 wallpapers` · `0.1.0` · clean monoline sibling: **Core Builds Icon Pack**
...
**The same 943 icons, drawn as pop art.** Android TV's only cartoon icon pack.
`943 icons` · `12 wallpapers` · `16 swatches` · `v1.0.0`
...
|---|---|
| One superellipse container on all 943 | no container; every launcher looked different |
...
The biggest single legibility win is the third rule. 549 of the 943 icons are
...
keyline with their own icon composited inside — so the answer to "943 icons" is
...
```bash
python tools/build_pop.py              # 943 icons + 943 banners + branding (~5 min)
...
nobody ships it on TV. Coverage was already won here at 943 icons; style was
the open axis.
...
tools/validate.py            coherence checks (25,000+ at 943 icons)

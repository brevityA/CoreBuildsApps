...
**GitHub Actions:** the workflow YAML is ready at `.github/workflows/core-line-apk.yml` in this repo (it needs one manual push by the repo owner — the build bot cannot push `workflows/` files). Once it is on the branch, open **Actions → Core Line APK → Run workflow** (or just push a `ticker/**` change) and download the `CoreLine-debug` artifact → sideload `app-debug.apk`.

The same YAML is kept in sync at `github-workflow-core-line-apk.yml` here, so a fresh lift of `ticker/` still carries it.

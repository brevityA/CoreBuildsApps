...
**Follow-up to 1.8.10: the rest of the D-pad dead ends, plus the checks that
should have caught them.**

### Fixed
- **Four more controls unreachable by D-pad.** 1.8.10 fixed the Wallpapers
  button by rewiring a single `nextFocus` edge; the same bug class survived
  in four more places, in all three packs. The update **Download/Install**
  button was reachable only via the one-shot focus grab when the update bar
  appeared — once focus moved, the remote could never return to it, so an
  available update could not be installed. The **"Also &lt;launcher&gt;"** row was
  never reachable at all when a second supported launcher was installed. On
  the wallpapers screen, the on-screen **Back** button was unreachable, and
...
- **Wallpaper chips lost their highlight on press.** `WallpaperChipAdapter`
  ...
  and the click target across two different views; the focusable card is now
  the root, with the selection ring as a non-focusable overlay.
...
- **Focus-reachability check in `check_ui_resources.py`.** A fifth static
  ...
  over. It flags all four regressions above *and* the original 1.8.10 one
  when run against the pre-fix layouts — nothing in CI would have caught
  either before.
...
wallpapers**. versionName 1.8.11, versionCode 21.

...
- `.github/workflows/suite-ci.yml`
  ...
  - `android`: matrix builds every Android app (`Icon Pack`, `Core Builds Banners`, `Core Line`, `Core Shift`, `Core Doctor`, `Core Motion plugin`) with `lintDebug`, unit tests, and `assembleDebug`, then uploads the debug APK artifact.
...
- Existing per-app workflows remain for compatibility with current release tags and Downloader flows:
  - `build.yml` — Icon Pack (+ Banners companion).
